...
import VoiceAudioSettingsPanel from "../shared/audio/VoiceAudioSettingsPanel.jsx";
...
export default function SettingsPage({
  ...
  canShowAdmin = false,
  onOpenAdmin,
  launcherSectionId = "general",
  visibleSections = null,
  pageTitle = "Settings",
  pageDescription = "Manage account, preferences, and support.",
  ...
  const requestedLauncherSectionId = String(launcherSectionId || "general").trim().toLowerCase() || "general";
  const visibleSectionSet = useMemo(
    () => (Array.isArray(visibleSections) ? new Set(visibleSections.map((value) => String(value || "").trim().toLowerCase())) : null),
    [visibleSections]
  );
  ...
  const isVisible = (sectionId) =>
    !visibleSectionSet || visibleSectionSet.has(String(sectionId || "").trim().toLowerCase());
  ...
  return (
    <div
      className="min-h-[100dvh] bg-slate-100 dark:bg-slate-950"
      data-launcher-section={requestedLauncherSectionId}
    >
      <div className="mx-auto max-w-5xl p-4 md:p-8">
        <div className="mb-6 flex flex-col gap-2">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300">{pageDescription}</p>
          {showStatusBadges ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          {isVisible("voice-audio") ? (
          <Section
            title="Voice & Audio"
            description="Choose the default AI voice, captions, speech speed, and reply playback behavior."
          >
            <VoiceAudioSettingsPanel uid={user?.uid || null} />
          </Section>
          ) : null}
          ...
          {canShowAdmin && isVisible("administration") ? (
