  ...
  return (
    <>
      <div className="space-y-5">
        <section className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white px-5 py-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:flex-row md:items-start md:justify-between md:px-6">
        ...
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
          {metrics.map(([label, value, note, Icon, toneKey]) => <article key={label} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-medium text-slate-600 dark:text-slate-300">{label}</div><div className="mt-3 text-3xl font-semibold tracking-tight text-slate-950 dark:text-white">{value}</div></div><span className={`inline-flex rounded-2xl border p-2 ${tone(toneKey)}`}><Icon size={16} /></span></div><div className="mt-3 text-xs text-slate-500 dark:text-slate-400">{note}</div></article>)}
        </section>
        ...
        <section className="space-y-3">
          ...
          {session ? <section className="rounded-3xl border border-slate-200 bg-white px-5 py-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between"><div className="space-y-3"><div className="flex flex-wrap items-center gap-3"><h2 className="text-lg font-semibold text-slate-950 dark:text-white">{session.title}</h2><Pill value={session.status} toneKey={session.tone} /></div><div className="grid grid-cols-1 gap-2 text-sm text-slate-600 dark:text-slate-300 md:grid-cols-2 xl:grid-cols-4"><Field label="Unit / Class" value={session.unit} /><Field label="Lecturer" value={session.lecturer} /><Field label="Date" value={fmt(session.date)} /><Field label="Time" value={session.time} /></div></div><div className="flex flex-wrap gap-2 xl:max-w-[540px] xl:justify-end">{[{ label: "Generate QR", icon: QrCode }, { label: "Show Code", icon: Eye }, { label: "Copy Link", icon: Copy }, { label: "Download QR", icon: Download }, { label: "Open Live Monitor", icon: CalendarDays }, { label: "End Session", icon: AlertCircle, danger: true }].map((item) => <button key={item.label} type="button" onClick={() => setAction(item.label)} className={["inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-medium", item.danger ? "border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100 dark:border-rose-400/20 dark:bg-rose-500/10 dark:text-rose-200" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-800"].join(" ")}><item.icon size={15} />{item.label}</button>)}</div></div></section> : null}
        </section>

        <section className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-5">
            <section className="rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between"><div><div className="text-lg font-semibold text-slate-950 dark:text-white">Attendance Records</div><div className="text-sm text-slate-500 dark:text-slate-400">Structured register for session-level attendance review, student check-ins, and follow-up actions.</div></div><div className="text-sm text-slate-500 dark:text-slate-400">Select a student row to open detailed attendance analytics.</div></div>
              <div className="overflow-x-auto"><table className="min-w-[1320px] text-sm"><thead className="sticky top-0 z-10 bg-slate-50 text-left text-slate-500 dark:bg-slate-950 dark:text-slate-400"><tr><th className="px-4 py-3 font-medium"><input type="checkbox" checked={allSelected} onChange={() => setChecked(allSelected ? checked.filter((id) => !filtered.some((row) => row.id === id)) : Array.from(new Set([...checked, ...filtered.map((row) => row.id)])))} className="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-400" /></th><th className="px-4 py-3 font-medium">Student</th><th className="px-4 py-3 font-medium">Registration No.</th><th className="px-4 py-3 font-medium">Email</th><th className="px-4 py-3 font-medium">Gender</th><th className="px-4 py-3 font-medium">Program / Class</th><th className="px-4 py-3 font-medium">Session</th><th className="px-4 py-3 font-medium">Date</th><th className="px-4 py-3 font-medium">Check-in Time</th><th className="px-4 py-3 font-medium">Status</th><th className="px-4 py-3 font-medium">Method</th><th className="px-4 py-3 font-medium">Actions</th></tr></thead><tbody>{filtered.map((row) => <tr key={row.id} className={["border-t border-slate-100 dark:border-slate-800", selected?.id === row.id ? "bg-sky-50/70 dark:bg-sky-500/10" : "bg-white dark:bg-slate-900"].join(" ")}><td className="px-4 py-3 align-top"><input type="checkbox" checked={checked.includes(row.id)} onChange={() => setChecked((s) => s.includes(row.id) ? s.filter((id) => id !== row.id) : [...s, row.id])} className="mt-1 h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-400" /></td><td className="px-4 py-3"><button type="button" onClick={() => setSelected(row)} className="text-left"><div className="font-medium text-slate-900 dark:text-white">{row.studentName}</div><div className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{row.department}</div></button></td><td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-200">{row.registrationNo}</td><td className="px-4 py-3 text-slate-600 dark:text-slate-300">{row.email}</td><td className="px-4 py-3 text-slate-600 dark:text-slate-300">{row.gender}</td><td className="px-4 py-3 text-slate-600 dark:text-slate-300">{row.programClass}</td><td className="px-4 py-3 text-slate-600 dark:text-slate-300"><div>{row.unit}</div><div className="text-xs text-slate-400 dark:text-slate-500">{row.session}</div></td><td className="px-4 py-3 text-slate-600 dark:text-slate-300">{fmt(row.date)}</td><td className="px-4 py-3 text-slate-600 dark:text-slate-300">{row.checkInTime}</td><td className="px-4 py-3"><Pill value={row.status} toneKey={statusTone(row.status)} /></td><td className="px-4 py-3"><Pill value={row.method} toneKey={row.method === "QR" ? "emerald" : row.method === "Code" ? "amber" : row.method === "Link" ? "sky" : "slate"} /></td><td className="px-4 py-3"><div className="flex items-center gap-2"><button type="button" onClick={() => setSelected(row)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"><Eye size={14} />View</button><button type="button" className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"><MoreHorizontal size={14} />More</button></div></td></tr>)}</tbody></table></div>
            </section>
